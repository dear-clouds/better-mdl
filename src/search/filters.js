...
// Checks if the page is already loaded and, if so, runs the script immediately
      ...
      if (mutation.type === 'childList') {
          ...
          for (let node of mutation.addedNodes) {
              if (node.nodeType === Node.ELEMENT_NODE && 
                  Array.from(node.classList).some(className => className.startsWith('mdl-'))) {
                  runScript();
              }
          }
...
observer.observe(document.body, {
  childList: true,
  subtree: true // ensures observing all descendants too
});
